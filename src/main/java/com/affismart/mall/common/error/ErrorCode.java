...
public enum ErrorCode {
	...
	PAYOUT_STATUS_TRANSITION_NOT_ALLOWED(
			"PAYOUT_STATUS_TRANSITION_NOT_ALLOWED",
			"Payout status transition is not allowed",
			HttpStatus.CONFLICT
	),
	PAYOUT_AMOUNT_BELOW_MINIMUM(
			"PAYOUT_AMOUNT_BELOW_MINIMUM",
			"Payout amount is below the minimum threshold",
			HttpStatus.BAD_REQUEST
	),
	PAYOUT_BALANCE_INSUFFICIENT(
			"PAYOUT_BALANCE_INSUFFICIENT",
			"Affiliate balance is not sufficient for payout request",
			HttpStatus.CONFLICT
	),
	...
	PAYMENT_REFUND_FAILED(
			"PAYMENT_REFUND_FAILED",
			"Failed to refund payment for order cancellation",
			HttpStatus.BAD_GATEWAY
	),
